  ...
  return (
    <div className='flex flex-col w-full mx-auto mt-20'>
      ...
      <div className='w-[100vw] flex px-20'>
        ...
        >
          {currentPlaylists && currentPlaylists.map((item: Playlist, index: number) => (
            <SwiperSlide key={index}>
              <div className='flex flex-col'>
                <img src={item.images[0].url} className="h-[250px] object-cover object-center cursor-grab rounded-lg"/>
                <span className='font-artists text-xl mt-4 leading-5 text-[#010101] font-medium tracking-[-0.2px]'>{item.name}</span>
              </div>
            </SwiperSlide>
          ))}
        </Swiper>
