...
export default function TrendingPlaylist() {
  ...
  const getTrendingPlaylist = () => {
    getSpotifyData({
      token: session?.accessToken as string,
      searchParams: currentCountry ? { country: currentCountry, limit: currentLimit } : { limit: currentLimit },
      queryLink: 'browse/categories/0JQ5DAqbMKFQIL0AXnG5AK/playlists',
    }).then((data: Data): void => {
      setTrendingPlaylist(data?.playlists?.items);
    });
  };
  ...
  useEffect(() => {
    session?.accessToken && getTrendingPlaylist();
  },[session?.accessToken])
    ...
    <div>
      <div className="flex flex-col w-full mx-auto mt-20">
        ...
        <div className="w-[100vw] flex px-20">
          ...
          >
            {trendingPlaylist &&
              trendingPlaylist.map((item: Playlist, index: number) => (
